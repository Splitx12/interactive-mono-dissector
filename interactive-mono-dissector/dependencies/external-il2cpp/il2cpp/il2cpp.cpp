...
auto il2cpp::types::il2cpp_t::get_module(const char* const module_name) const
{
...
std::optional<std::reference_wrapper<const std::pair<std::string, il2cpp::types::il2cpp_image_t>>> il2cpp::types::il2cpp_t::get_image(const std::string& image_name) const
{
    const auto images_iterator = std::ranges::find(images, image_name, &std::pair<std::string, il2cpp_image_t>::first);

    return images_iterator == images.cend() ? std::nullopt : std::make_optional(std::ref(*images_iterator));
}
