...
namespace il2cpp::types
{
	class il2cpp_t
	{
		...
		auto get_module(const char* const module_name) const;
	...
	public:
		...
		auto read(const std::uintptr_t address) const
		{
			buffer_t buffer;
		...
		std::optional<std::reference_wrapper<const std::pair<std::string, il2cpp::types::il2cpp_image_t>>> get_image(const std::string& image_name) const;
